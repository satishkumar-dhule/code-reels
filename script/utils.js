...
/**
 * Format a bot comment for GitHub Discussions
 */
function formatBotComment(botName, changeType, details) {
  const timestamp = new Date().toISOString();
  const emoji = getBotEmoji(changeType);
  
  let body = `### ${emoji} ${botName} Update\n\n`;
  body += `**Change Type:** ${formatChangeType(changeType)}\n`;
  ...
  if (details.before && details.after) {
    ...
    body += `**Before:**\n\`\`\`\n${truncateText(details.before, 500)}\n\`\`\`\n\n`;
    body += `**After:**\n\`\`\`\n${truncateText(details.after, 500)}\n\`\`\`\n`;
    body += `</details>\n\n`;
  }
  
  body += `---\n*This comment was automatically generated by the ${botName}.*`;
